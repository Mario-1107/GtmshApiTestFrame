...
class member():
    ...
    def get_token(self):
        ...
        payload = {'custid': self.custid, 'act': 'GetToken', 'random': self.randoms, 'username': username,
                   'password': password}
    ...
    def get_VipInfoByTele(self, mobile):
        ...
        '''
        url = self.sit_url + 'ThirdApiHandler/VipHandler.ashx'
        payload = {'custid': self.custid, 'act': 'GetVipInfoByTele', 'random': self.randoms, 'token': self.token,
                   'mobile': mobile}
        response = requests.request("POST", url, headers=self.headers, data=payload)
    ...
    def scoreOperate(self, cardnumber, score='1000', type='101'):
        ...
        url = self.sit_url + 'ThirdApiHandler/VipHandler.ashx'
        payload = {'custid': self.custid, 'act': 'ScoreOperate', 'random': self.randoms, 'token': self.token,
                   'cardnumber': cardnumber, 'score': score, 'type': type}
        response = requests.request("POST", url, headers=self.headers, data=payload)
        logger.info("请求参数：" + str(payload) + ";接口响应结果：" + response.text)
    ...
    def vip_FillMoney(self, cardnumber, fillmoney=1000, type=0):
        ...
        if len(cardnumber) == 11:
            cardnumber = self.get_VipInfoByTele(cardnumber)
        url = self.sit_url + 'ThirdApiHandler/VipHandler.ashx'
        payload = {'custid': self.custid, 'act': 'VipFillMoney', 'random': self.randoms, 'token': self.token,
                   'cardnumber': cardnumber, 'fillmoney': fillmoney, 'transaction_id': self.randoms,
                   'out_trade_no': self.randoms, 'type': type}
        response = requests.request("POST", url, headers=self.headers, data=payload)
        logger.info("请求参数：" + str(payload) + ";接口响应结果：" + response.text)
    ...
    def get_DynamicCode(self, cardnumber):
        ...
        if len(cardnumber) == 11:
            cardnumber = self.get_VipInfoByTele(cardnumber)
        url = self.sit_url + 'ThirdApiHandler/VipHandler.ashx'
        payload = {'custid': self.custid, 'act': 'GetDynamicCode', 'random': self.randoms, 'token': self.token,
    ...
    def get_Bills(self, branchno, tableno, unionid='oxvw21QlHBkA289XhFbHIL1MolJw'):
        ...
        '''
        url = self.sit_url + 'ThirdApiHandler/VipHandler.ashx'
        payload = {'custid': self.custid, 'act': 'GetBills', 'random': self.randoms, 'token': self.token,
                   'unionid': unionid, 'branchno': branchno, 'tableno': tableno}
        response = requests.request("POST", url, headers=self.headers, data=payload)
        logger.info("请求参数：" + str(payload) + ";接口响应结果：" + response.text)
    ...
    def get_BillsByThirdNo(self, branchno, thirdno):
        ...
        '''
        url = self.sit_url + 'ThirdApiHandler/VipHandler.ashx'
        payload = {'custid': self.custid, 'act': 'GetBillsByThirdNo', 'random': self.randoms, 'token': self.token,
                   'branchno': branchno, 'thirdno': thirdno}
        response = requests.request("POST", url, headers=self.headers, data=payload)
        logger.info("请求参数：" + str(payload) + ";接口响应结果：" + response.text)
    ...
    def get_vipinfo(self, unionid='oxvw21QlHBkA289XhFbHIL1MolJw'):
        ...
        '''
        url = self.sit_url + 'ThirdApiHandler/VipHandler.ashx'
        payload = {'custid': self.custid, 'act': 'GetVipInfo', 'random': self.randoms, 'token': self.token,
                   'unionid': unionid}
        response = requests.request("POST", url, headers=self.headers, data=payload)
        logger.info("请求参数：" + str(payload) + ";接口响应结果：" + response.text)
    ...
    def get_AdvancedVipInfo(self, unionid='oxvw21QlHBkA289XhFbHIL1MolJw', type='0'):
        ...
        '''
        url = self.sit_url + 'ThirdApiHandler/VipHandler.ashx'
        payload = {'custid': self.custid, 'act': 'GetAdvancedVipInfo', 'random': self.randoms, 'token': self.token,
    ...
    def submitOrderMeal(self, branchno='5999'):
        ...
        '''
        url = self.sit_url + 'ThirdApiHandler/VipHandler.ashx'
        payload = {'custid': self.custid, 'act': 'SubmitOrderMeal', 'random': self.randoms, 'token': self.token,
                   'branchno': branchno,
    ...
    def get_VipIntegralFlowList(self, cardnumber, begindate=now_time, enddate=now_time, pageindex=1, pagesize=100):
        ...
        if len(cardnumber) == 11:
            cardnumber = self.get_VipInfoByTele(cardnumber)
        url = self.sit_url + 'ThirdApiHandler/VipHandler.ashx'
        payload = {'custid': self.custid, 'act': 'GetVipIntegralFlowList', 'random': self.randoms, 'token': self.token,
                   'cardnumber': cardnumber, 'pageindex': pageindex, 'pagesize': pagesize, 'begindate': begindate,
                   'enddate': enddate}
        response = requests.request('POST', url, headers=self.headers, data=payload)
        logger.info("请求参数：" + str(payload) + ";接口响应结果：" + response.text)
